...
public interface PublicacaoRepository extends PagingAndSortingRepository<Publicacao, BigDecimal> {
    ...
    @Query("SELECT pc FROM PublicacaoConteudo pc JOIN pc.idPublicacao pu WHERE pu.idPerfil = :perfil ORDER BY pu.dtPublicacao")
    PublicacaoConteudo findPublicacoesUsuario(@Param("perfil") Perfil perfis);

    @Query("SELECT pc FROM PublicacaoConteudo pc JOIN pc.idPublicacao pu WHERE pu.idPerfil IN :perfis ORDER BY pu.dtPublicacao")
